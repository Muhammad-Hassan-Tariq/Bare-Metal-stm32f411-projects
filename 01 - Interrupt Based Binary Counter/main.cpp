...
void led_pattern() {
  for (int i = 0; i < 5; i++) {
    uint32_t targetState = (count >> i) & 1; // Isolate the i-th bit of count (0 or 1)
    uint32_t actualState = (GPIOA->IDR >> i) & 1; // Isolate the i-th bit of the physical pin (0 or 1)
...
extern "C" void EXTI0_IRQHandler(void) {
  if (EXTI->PR & EXTI_PR_PR0) { 
    EXTI->PR = EXTI_PR_PR0; // Reset Pending Register so Interrupt doesn't retrigger
    count++; // Increment Count

    // This is for demonstration purposes. In real projects try to keep Interrupt Handlers as minimal as possible
    // Best approach is to just update flags here & carry out remaining process inside main function
    led_pattern();
  }
}
